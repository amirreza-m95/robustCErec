...
from source.utils import get_adj_matrix, edges_filter_nodes
...
class LightGCN(nn.Module):
    ...
    def forward(self, user_ids, item_ids):
        ...
        prediction = torch.sum(user_embed * item_embed, dim=1) + user_bias.squeeze() + item_bias.squeeze()
        return prediction

def train_lightgcn(model, train_data, num_epochs, learning_rate):
